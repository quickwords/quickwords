    ...
    <div class="bg flex h-screen font-sans" :class="['bg-black text-grey-light', 'bg-image text-grey-darkest'][theme]"
        ...
        <div class="flex flex-col flex-2 p-8">
            <h1 class="flex items-center h-12">
                <router-link :to="{ name: 'Snippets' }" class="text-2xl text-grey-dark cursor-pointer no-underline">
                    Snippets
                </router-link>
                ...
                <div class="mb-8 flex-1">
                    ...
                    >
                        ...
                        <div class="w-16 flex rounded overflow-hidden">
                            <div class="w-16 py-1 shadow-inner text-grey-darkest select-none flex-no-shrink text-center bg-blue-light transition"
                                 :class="{ '-ml-64': theme }">dark
                            </div>
                            <div class="w-16 py-1 flex-no-shrink select-none text-center bg-grey-dark text-grey-lightest">
                                light
                            </div>
                        </div>
                    </label>
                    ...
                    >
                        ...
                        <icon-checkbox v-if="autoLaunch" :checked="true"
                                       class="w-6 h-6 text-blue-light fill-current"></icon-checkbox>
                    ...
                    >
                        <span class="select-none" id="update_label"
                              aria-label="Check for Updates">Check for Updates</span>
                        <input type="checkbox" class="invisible" v-model="autoUpdate">

                        <icon-checkbox v-if="autoUpdate" :checked="true"
                                       class="w-6 h-6 text-blue-light fill-current"></icon-checkbox>
                        <icon-checkbox v-else :checked="false" class="w-6 h-6 fill-current"></icon-checkbox>
                    </label>
        ...
        <div class="flex-3 p-8 flex flex-col relative shadow-lg" :class="section ? 'active' : ''">
            ...
            <div class="flex justify-center items-center logo" @click="edit(null)"
                 :class="{ 'cursor-pointer': section !== null }">
