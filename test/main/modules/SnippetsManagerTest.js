...
describe('SnippetsManager', () => {
    describe('constructor', () => {
        ...
        it('sets keyboard handlers', () => {
            ...
            let onWithKeyDownCalledWith = undefined
            let onWithMouseClickCalledWith = undefined
            ...
            const snippetsManager = new SnippetsManager({ store, keyboardHandler, keyboardSimulator, clipboard })
            snippetsManager._onKeyDown = () => { called = true }

            assert.equal(true, called)
            assert.equal(true, _.isFunction(onWithKeyDownCalledWith))
            // assert.equal(snippetsManager._onMouseClick, onWithMouseClickCalledWith)
    ...
    describe('_onKeyDown', () => {
        ...
        it('calls appropriate methods if a key is resolved', () => {
            let _addCharToBufferCalled = false
            let _shortenBufferIfNecessaryCalled = false
            let _replaceSnippetIfMatchFoundCalled = false

            snippetsManager._addCharToBuffer = (character) => { _addCharToBufferCalledWith = character }
            snippetsManager._shortenBufferIfNecessary = () => { _shortenBufferIfNecessaryCalled = true }
            snippetsManager._replaceSnippetIfMatchFound = () => { _replaceSnippetIfMatchFoundCalled = true }
